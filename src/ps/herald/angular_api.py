...
def list():
    ...
    if query_parameters["system_id"] != "not_selected":
        query_obj= query_obj.filter( Log.system_id == query_parameters["system_id"])
    if query_parameters["sub_system_id"] != "not_selected":
        query_obj= query_obj.filter( Log.sub_system_id == query_parameters["sub_system_id"])
    if query_parameters["sub_sub_system_id"] != "not_selected":
        query_obj= query_obj.filter( Log.sub_sub_system_id == query_parameters["sub_sub_system_id"])
    if query_parameters["user_spec_1"] != "not_selected":
        query_obj= query_obj.filter( Log.user_spec_1 == query_parameters["user_spec_1"])
    if query_parameters["user_spec_2"] != "not_selected":
        query_obj= query_obj.filter( Log.user_spec_2 == query_parameters["user_spec_2"])
...
def options():
    ...
    res = {}
    res["produkt_ids"] = [x[0] for x in  session.query(Log.produkt_id).distinct().all()]
    res["produkt_ids"].append("not_selected") 
    res["system_ids"] = [x[0] for x in session.query(Log.system_id).distinct().all()]
    res["system_ids"].append("not_selected") 
    res["sub_system_ids"] = [x[0] for x in session.query(Log.sub_system_id).distinct().all()]
    res["sub_system_ids"].append("not_selected") 
    res["sub_sub_system_ids"] = [x[0] for x in session.query(Log.sub_sub_system_id).distinct().all()]
    res["sub_sub_system_ids"].append("not_selected") 
    res["user_spec_1s"] = [x[0] for x in session.query(Log.user_spec_1).distinct().all()]
    res["user_spec_1s"].append("not_selected") 
    res["user_spec_2s"] = [x[0] for x in session.query(Log.user_spec_2).distinct().all()]
